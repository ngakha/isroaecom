    ...
    { key: 'phone', label: t('common.phone'), render: (row) => row.phone || '-' },
    {
      ...
      render: (row) => (
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${row.is_active !== false ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {row.is_active !== false ? t('customers.active') : t('customers.inactive')}
        </span>
      ),
    },
    {
      ...
      render: (row) => row.last_login ? new Date(row.last_login).toLocaleDateString() : t('customers.never'),
    },
    {
      key: 'created_at',
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSearch} className="flex gap-2 max-w-md">
        <div className="relative flex-1">
          ...
          <input type="text" className="input pl-9" placeholder={t('customers.searchPlaceholder')} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      <Modal isOpen={!!selectedCustomer} onClose={() => setSelectedCustomer(null)} title={t('customers.customerDetails')} size="lg">
        {selectedCustomer && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500">{t('customers.registered')}</p>
            ...
            {selectedCustomer.addresses?.length > 0 && (
              <div>
                <h4 className="font-semibold mt-4 mb-2">{t('customers.addresses')}</h4>
                <div className="space-y-2">
                  {selectedCustomer.addresses.map((addr) => (
                    <div key={addr.id} className="p-3 bg-gray-50 rounded-lg text-sm">
                      <p className="font-medium">{addr.label} {addr.is_default && `(${t('customers.default')})`}</p>
      ...
      {/* Edit Customer Modal */}
      <Modal isOpen={!!editModal} onClose={() => setEditModal(null)} title={t('customers.editCustomer')}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm">{t('customers.active')}</span>
          </label>
